...
/**
 * The {@link Animated} class is a subclass of the {@link JFrame} which
 * implements the {@link SimulatorView} interface.
 * 
 * <p>
 * The {@link Animated} class is responsible for creating and displaying visual
 * representations of the {@link Station} class. The {@link Animation} also
 * works independently as the {@link #animator} asynchronously displays the
 * {@link Station} objects.
 * </p>
 * 
 * <p>
 * The {@link Animated} also allows the simulation to be paused and the speed of
 * which the {@link #animator} displays the simulation can also be specified.
 * </p>
 * 
 * <p>
 * The {@link Animated} class uses the singleton pattern to allow only one
 * instance of the {@link Animated} to exist.
 * </p>
 * 
 * @author John_Berg
...
public final class Animated extends JFrame implements SimulatorView<Station> {
	 ...
	 * @see Collectors
	 */
	private static final List<Class<? extends RoadUser>> roadUsers = Stream
			.of(SmallCar_RoadUser.class, Motorbike_RoadUser.class, FamilySedan_RoadUser.class, Truck_RoadUser.class)
	...
	/**
	 * The {@link Thread} object which is timing the updates of the view to run
	...
	/**
	 * {@link JButton} responsible for controlling the {@link Thread} object's
	 * state.
	 * 
	 * @see JButton
	 */
	...
	/**
	 * Constructs a {@link JFrame} that holds a newly constructed
	 * {@link AnimationPanel} and provides a panel underneath with buttons and
	 * sliders to control the speed and state of the animation,It then uses
	 * methods from {@link SimulatorView} to allow the simulation to be
	 * represented with the user chosen values from {@link UserInterface}
	 * 
	 * <p>
	...
	private Animated() {
		...
		// Creates a new button and sets the aprpriate text for both Control and
		// Kill buttons.
		...
		// format tyhe speed sliders tick spacing and then paints the ticks and
		// labels.
		...
		killSwitch.addActionListener(e -> {
			// When this action listener is called call the dispose function and
			// then system exit.
			dispose();
			System.exit(0);
		});
		...
		/*
		 * Initialise the animator Thread with a Runnable object which holds the
		 * routine that the Thread will follow to update the mainPanel.
		 */
		animator = new Thread(() -> {
			...
			while (true) {
				...
				if (!Thread.currentThread().isInterrupted()) {
					...
					try {
						...
						// Get the value of the speedSlider and calculate the
						// update rate.
						updateRate = TEN_SECOND / speedSlider.getValue();
					} catch (ArithmeticException e) {

						/*
						 * If calculating the updateRate results in division by
						 * 0, then set the updateRate to TEN_SECOND.
						 */
						updateRate = TEN_SECOND;
					}

					/*
					 * Calculate the difference (delta) in time between the
					 * current time and the previousTime, while the delta is
					 * greater than or equal to the updateRate then enter the
					 * loop; upon loop completion, subtract the updtateRate from
					 * delta and then repeat if the condition is still true.
					 * 
					 * This loop allows the thread to update the AnimationPanel
					 * a set number of times per second, where if the thread
					 * completed the task quickly, the thread must wait until
					 * enough time has elapsed to update again; and if the
					 * thread is not keeping up with the number of updates per
					 * second, then the thread will catch up update the amount
					 * of times possible, since the number of times the thread
					 * will repeat the loop increases.
					 */
					for (long delta = System.nanoTime() - previousTime; updateRate < delta; delta -= updateRate) {
						...
						previousTime = System.nanoTime(); // Update the previous
															// time.
					}
				} else {

					/*
					 * Suspend the thread, then when the thread is resumed,
					 * remove the interrupt flag and set the previousTime to the
					 * current nanoTime.
					 */
		...
		// Set the animator Thread priotiry to the lowest possible, then start
		// the animator.
	...
	/**
	 * Control the execution of the {@link #animator} by interrupted and
	 * suspending the {@link Thread} or resuming the {@link Thread}.
	 * 
	 * <p>
	 * Activated when the {@link #controlButton} is clicked checks if the
	 * {@link animator} is interrupted and if so, then call the
	 * {@link Thread#resume()} method to allow the {@link #animator} to
	 * continiue execution, otherwise interrupt the {@link #animator}.
	 * </p>
	 * 
	 * <p>
	 * Set the title of the {@link #controlButton} to "Pause" or "Continue"
	 * depending on if the {@link #animator} is running or interrupted
	 * respectively.
	 * </p>
	 * 
	 * @see #animator
	...
	private void control() {
		...
		if (!animator.isInterrupted()) {
			/*
			 * if the animator thread is not interrupted change the text on the
			 * control button and interrupt the thread.
			 */
			controlButton.setText("Continue");
			animator.interrupt(); // Set the interrupt flat of the animator.
		} else {

			// else set the control button text to pause and resume the
			// interrupted thread.
	 ...
	 * 
	 * <p>
	 * If the {@link #animated} is <code>null</code> then initialise a new
	 * instance of {@link Animated} before returning it.
	 * </p>
	 * 
	 * <p>
	 ...
	 * @param station
	 *            The {@link Station} to be displayed.
	 */
	@Override
	...
	public void setEnd() {
		...
		/*
		 * Add a new action listener to the killSwitch which will stop the
		 * animator and dispose of the Animation, then create a new frame with
		 * the results listed as a table.
		 */
		killSwitch.addActionListener(e -> {
			...
			/*
			 * Create the StringBuilders which will be used to append
			 * information about the simulation as a table, each StringBuilder
			 * represents a row.
			 */
			...
			// Create the formating of the money and counters to not overflow
			// the table columns.
			DecimalFormat money = new DecimalFormat("#.##");
			DecimalFormat counter = new DecimalFormat("####");

			// Iterate through the RoadUsers and append the information to the
			// results table.
			roadUsers.forEach(r -> {

				// Add The RoadUser class name to the headers.
				headers.append(r.getSimpleName().replaceFirst("_RoadUser", "")).append("\t");
				// Add the number of processed vehicles of that type.
				processed.append(counter.format(endStation.getRoadUsersProcessed().get(r))).append("\t");
				// Add the number oh rejected vehicles of that type.
				rejected.append(counter.format(endStation.getRoadUsersRejected().get(r))).append("\t");
				// Add the fuel profit of that vehicle.
				fuelProfit.append(money.format(endStation.getFuelProfit().get(r))).append("\t");
				// Add the lost fuel profit of the vehicle.
				lostFuelProfit.append(money.format(endStation.getLostFuelProfit().get(r))).append("\t");
				// Add the shop profit of the road user.
				shopProfit.append(money.format(endStation.getSalesProfit().get(r))).append("\t");
				// Add the lost shop profit of that road user.
				lostShopProfit.append(money.format(endStation.getLostSalesProfit().get(r))).append("\t");
			});
			...
			/*
			 * Add the JTextArea to the centre of the Results frame, then pack
			 * the components and make the frame visible.
			 */
	...
	/**
	 * The {@link TimeStamp} class is a container used for the {@link Animation}
	 * class to store {@link Station} objects and times.
	 * 
	 * @author John Berg
	 * @version 20/03/2017
	 * @since 20/03/2017
	...
	private final class TimeStamp {
		...
		/**
		 * The snapshot version of the {@link Station} objects current state.
		 */
	...
	/**
	 * The {@link AnimationaPanel} class is a subclass of the {@link JPanel}
	 * class which is designed for drawing visual representations of the
	 * {@link Station} objects.
	 * 
	 * <p>
	...
	public static class AnimationPanel extends JPanel {
		...
		/**
		 * The amount of space between the main area of the
		 * {@link AnimationPanel} and the left side of {@link AnimationPanel}.
		 * 
		 * <p>
		 * The {@link #LEFT_MARGIN} reserves space for information about the
		 * simulation to be displayed.
		 * </p>
		 */
		private static final int LEFT_MARGIN = 170;
		/**
		 * The size of the area which is usable when drawing the
		 * {@link Location} objects.
		 * 
		 * <p>
		 * Each {@link Location} is assigned an area of equal size, where the
		 * size is also used to organise how the {@link Location} objects are
		 ...
		 * 
		 * <p>
		 * The default {@link Font} which is used by the {@link AnimationPanel}
		 * and the size of the {@link Font}.
		 * </p>
		 * 
		 * @see Font
		 */
		 ...
		 * 
		 * <p>
		 * The {@link Image} object which the {@link AnimationPanel} uses to
		 * draw the simulation.
		 * </p>
		 * 
		 * @see Image
		 ...
		 * 
		 * <p>
		 * Initialise a {@link AnimationPanel} by providing the initial width
		 * and height of the panel.
		 * </p>
		 * 
		 * @param width
		 *            The height of the {@link AnimationPanel}.
		 * @param height
		 *            The width of the {@link AnimationPanel}.
		 */
		 ...
		 * 
		 * <ol>
		 * <li>Initialise the {@link #img} if <code>null</code> or if the this
		 * panel's size has been changed</li>
		...
		private void prepareDraw() {
			...
			if (img == null || getWidth() != size.width || getHeight() != size.height) {
				...
				img = super.createImage(getWidth(), getHeight()); // Initialise
																	// the img.
			...
			g.fillRect(0, 0, size.width, size.height); // Draw the rectangle as
														// the background.
		...
		/**
		 * Draw a table of {@link RoadUser} classes with their associated
		 * {@link Color}.
		 * 
		 * <p>
		 ...
		 * 
		 * <p>
		 * The table is created at the bottom of the {@link AnimationPanel}
		 * where each entry is added as a row; if the entry exceeds the width of
		 * the {@link #img} then the entry will wrap to the next row of the
		 * table.
		 * </p>
		 * 
		 * @see Color
		...
		private final void drawInfo(final String[] lines) {
			...
			final int colorNameMargin = 10; // Spacing between the color and the
											// name.
			...
			for (String line : lines) {
				...
				g.drawString(line, getEffectiveWidth(), position += g.getFontMetrics().getHeight());
			}
			...
			for (Entry<Class<? extends RoadUser>, Color> ruc : Visualisation.getRoadUserColors()) {
				...
				// Set the color to the color of the roaduser, then draw an 8 x
				// 8 rectangle.
				g.setColor(ruc.getValue());
				g.fillRect(getEffectiveWidth(), position, 8, 8);

				/*
				 * Set the color to black and then draw the name of the roaduser
				 * next to the color.
				 */
				g.setColor(Color.black);
		...
		/**
		 * Get the width which the {@link AnimationPanel} can use to draw the
		 * visual representation of the simulation.
		 * 
		 * @return The usable width of the {@link AnimationPanel} for
		 ...
		 * <li>Grouping the {@link Location} objects
		 * ({@link #groupLocations(List)})</li>
		 * <li>Calculating the spacing between {@link Location} objects</li>
		 * <li>Drawing each {@link Location} from the grouped {@link Location}
		 * objects at the next position.</li>
		 * </ol>
		 * </p>
		 * 
		 * <p>
		 * The specific visual representation for the {@link Location} object is
		 * stored in the {@link Visualisation} enum, which can be accessed by
		 * calling the {@link Visualisation#getVisual(Class)} method.
		 * </p>
		 * 
		 * @param station
		 *            The {@link Station} to be drawn to the
		 *            {@link AnimationPanel}.
		 * @see Station
		 * @see Visualisation
		...
		public void draw(final int time, final Station station) {
			...
			for (List<Location> group : locationGroups) {
				...
				for (Location loc : group) {
					...
					// Set the color to black and then draw the name of the
					// location.
					g.setColor(Color.BLACK);
					g.drawString(loc.getClass().getSimpleName(), positionX, positionY);

					// Get the visual representation of the location and draw
					// it.
					Visualisation.getVisual(loc.getClass()).visual.visiulise(g, loc, positionX, positionY);
			...
			drawInfo(new StringBuilder().append("Time:   ").append(time).append("\n").append(station).toString()
					.split("\n"));
			repaint();
		}

		/**
		 * Group a {@link List} of {@link Location} objects into a {@link List}
		 * objects of {@link Location} objects which exist in parallel.
		 * 
		 * <p>
		 * Convert a {@link List} of {@link Location} objects into a
		 * {@link List} of {@link List} of {@link Location} objects which
		 * represent the {@link Location} objects which exist in parallel during
		 * the simulation.
		 * </p>
		 * 
		 * <p>
		 * A groups of {@link Location} objects must share the same
		 * {@link Location#nextLocation}, {@link Location} objects which meet
		 * this requierment will be placed in the same group as the other
		 * {@link Location} object with the same next {@link Location};
		 * otherwise the {@link Location} is placed in the next group.
		 * </p>
		 * 
		 * @param locations
		 *            A {@link List} of {@link Location} objects.
		 * @return A {@link List} of {@link List} objects containing locations
		 *         which exist in parallel.
		 * @see List
		 * @see Location
		 */
		private List<List<Location>> groupLocations(List<Location> locations) {
			...
			/*
			 * Create a list which contains the a list of locations,
			 * representing the locations which exist in parallel.
			 */
			...
			for (Location loc : locations) {
				...
				if (locationGroups.peekLast().isEmpty()) {
					...
					/*
					 * If there is currently no location in the last group, then
					 * add the first location to the first group of locations.
					 * 
					 * Locations which exist in parallel must have the same next
					 * location.
					 */
					...
					/*
					 * The location is parallel with the locations in the same
					 * group and is therefore added to the group.
					 */
					locationGroups.peekLast().add(loc);
				} else {
	 ...
	 * classes. Since the {@link Visualisation} enum is responsible for
	 * representing the {@link Location} classes, it must also be responsible
	 * for {@link RoadUser} classes, as the {@link Location} objects contain the
	 * {@link RoadUser} objects.
	 * </p>
	 * 
	 * @author John Berg
	 * @version 13/04/2017
	...
	public enum Visualisation {
		 ...
		 * 
		 * <p>
		 * The procedure to produce the visual representation of the
		 * {@link Pump} class.
		 * </p>
		 * 
		 * <p>
		...
		PUMP(Pump.class, (g, l, x, y) -> {
			...
			final int vehicleQueuSize = 3; // The scaling factor of the
											// vehicles.

			/*
			 * Set the background color to gray, then draw a rectangle with
			 * dimensions pumpWidth x pumpHeight.
			 */
			g.setColor(Color.GRAY);
			g.fillRect(x, y + AnimationPanel.BLOCK_SIZE / 4, pumpWidth, pumpHeight);

			/*
			 * Draw an pumpSize x pumpSize black rectangle at the end of the to
			 * represent the pump.
			 */
			g.setColor(Color.BLACK);
			...
			for (RoadUser ru : l.getQueue()) {
				...
				/*
				 * Get the road user's vehicle, then get the size of the vehicle
				 * and scale it by vehicleQueueSize x 4.
				 * 
				 * Set the color to the roaduser's color. then draw a rectangle
				...
				g.fillRect(position - vehicleSize, y + pumpHeight, vehicleSize, vehicleHeight);
				position -= vehicleSize + vehicleHeight; // Move to the position
															// of the next
															// vehicle.
			}
		}),
		 ...
		 * 
		 * <p>
		 * The procedure to produce the visual representation of th {@link Till}
		 * class.
		 * </p>
		 * 
		 * <p>
		 ...
		 * <li>Drawing the {@link RoadUser} objects currently queueing to check
		 * out</li>
		 * </ol>
		 * </p>
		 * 
		...
		TILL(Till.class, (g, l, x, y) -> {
			...
			// Set the color to gray and then draw a square which represents the
			// Till area.
			g.setColor(Color.GRAY);
			g.fillRect(x + AnimationPanel.BLOCK_SIZE / 2, y + AnimationPanel.BLOCK_SIZE / 4, tillWidth, tillHeight);

			// Set the color to black and draw the till representation.
			g.setColor(Color.BLACK);
			g.fillRect(tillLocationX, tillLocationY, tillSizeX, tillSizeY);

			// Set the color to orange and draw a square to represent a
			// shopkeeper.
			...
			for (RoadUser ru : l.getQueue()) {
				...
				// Get the color of the roaduser and draw a square to represent
				// the roaduser.
				g.setColor(getColorOf(ru.getClass()));
				g.fillRect(position - roadUserSize, y + AnimationPanel.BLOCK_SIZE / 2, roadUserSize, roadUserSize);
				position -= roadUserSize + 2; // Move to the next position and
												// add a margin of 2.
			}
		}),
		/**
		 * The visual representation of any {@link Location} not included in the
		 * list.
		 * 
		 * <p>
		 * This {@link LocationVisual} is used for the {@link Location} classes
		 * which do not have a specific representation.
		 * </p>
		 * 
		 * <p>
		 * The {@link #DEFAULT} {@link Visualisation} will draw a square and
		 * then place all the {@link RoadUsers} at a random position inside the
		 * square to simulate movement.
		 * </p>
		 * 
		 * @see Visual
		 * @see #visual
		...
		DEFAULT(null, (g, l, x, y) -> {
			...
			l.getQueue().forEach(ru -> {
				...
				/*
				 * Set the color the the color of the RoadUser, then draw the
				 * roaduser at a random position inside the square.
				 */
				g.setColor(getColorOf(ru.getClass()));
				g.fillRect(x + roadUserSize + rng.nextInt(AnimationPanel.BLOCK_SIZE - 2 * roadUserSize),
						y + roadUserSize + rng.nextInt(AnimationPanel.BLOCK_SIZE - 2 * roadUserSize), roadUserSize,
						roadUserSize);
			});
		});
		 ...
		 * {@link Color} is set manually via the
		 * {@link #setColorOf(Class, Color)} method.
		 * 
		 ...
		 * @see RoadUser
		 */
		private static final Map<Class<? extends RoadUser>, Color> ROAD_USER_COLOR_MAP = new HashMap<Class<? extends RoadUser>, Color>();
		/**
		 * The {@link Location} subclass for which the visual representation is
		 * intended for.
		 * 
		 * @see Location
		 */
		 ...
		 * 
		 * <p>
		 * Create the {@link Visualisation} object by providing a
		 * {@link Location} class for which the {@link Visualisation} is
		 * associated with, and a {@link Visual} which contains the detaild
		 * implementation of producing the visual representation of the
		 * {@link Location}.
		 * </p>
		 * 
		 * @param locationClass
		 *            The {@link Location} class for which the
		 *            {@link Visualisation} is associated with.
		 * @param visual
		 *            The implementation for creating the visual representation
		 ...
		 * @see Visual
		 */
		private Visualisation(final Class<? extends Location> locationClass, final Visual visual) {
		...
		/**
		 * Set the color of a {@link RoadUser} class to a specified
		 * {@link Color}.
		 * 
		 * <p>
		 * Set the color of a {@link RoadUser} subclass to a specific
		 * {@link Color}, where neither the {@link RoadUser} or {@link Color}
		 * are null.
		 * </p>
		 * 
		 * @param roadUser
		 *            The {@link RoadUser} to set the color of.
		 * @param color
		 *            The {@link Color} to represent the {@link RoadUser} type.
		...
		public final void setColorOf(final Class<? extends RoadUser> roadUser, final Color color)
			...
			if (roadUser == null)
				throw new IllegalArgumentException("null is not a valid Roaduser class");
			if (color == null)
				throw new IllegalArgumentException("null is not a valid Color");
		 ...
		 * 
		 * <p>
		 * If the {@link RoadUser} class has not yet been assigned a
		 * {@link Color}, then get the {@link Color} at the current
		 * {@link #COLOR_INDEX} and increment the index by 1.
		 * </p>
		 * 
		 * @param roadUserClass
		 *            The {@link RoadUser} class to get the {@link Color}
		 *            representation of.
		 * @return The {@link Color} of the {@link RoadUser} class.
		 * @throws IllegalArgumentException
		 *             If the {@link RoadUser} class is null.
		 * @see #COLOR_INDEX
		 * @see #ROAD_USER_COLOR_MAP
		 * @see RoadUser
		 * @see Color
		...
		public static final Color getColorOf(final Class<? extends RoadUser> roadUserClass)
			...
			if (!ROAD_USER_COLOR_MAP.containsKey(roadUserClass)) {
				...
				ROAD_USER_COLOR_MAP.put(roadUserClass, USABLE_COLORS[COLOR_INDEX++]);

				/*
				 * If the COLOR_INDEX is out of bounds of the USABLE_COLORS the
				 * reset it to 0.
				 */
				if (!(COLOR_INDEX < USABLE_COLORS.length))
					COLOR_INDEX = 0;
			}
		 ...
		 * 
		 * <p>
		 * Reset all colors so that no {@link RoadUser} class has a
		 * {@link Color} representation. Reset the {@link #COLOR_INDEX} to
		 * <code>0</code>.
		 * </p>
		 * 
		 * @see #COLOR_INDEX
		...
		/**
		 * Get the {@link RoadUser} and the {@link Color} used in the
		 * {@link Visualisation}.
		 * 
		 ...
		 * 
		 * <p>
		 * Search for the {@link Visualisation} where the {@link #locationClass}
		 * matches the specified {@link Location} class. No match will result in
		 * the {@link #DEFAULT} {@link Visualisation} being selected.
		 * </p>
		 * 
		 * @param targetClass
		 *            The {@link Location} class to be used to search for the
		 *            correct {@link Visualisation}.
		 * @return The {@link Visualisation} associated with the
		 *         {@link Location} class, returns {@link #DEFAULT} if the
		 *         {@link Location} class does not match any
		 *         {@link #locationClass}.
		 */
		public static final Visualisation getVisual(final Class<? extends Location> targetClass) {
			...
			for (Visualisation lv : values()) {
				...
				/*
				 * If the targetClass matches the locationClass, then return
				 * that LocationVisual for which the target class matched.
				 */
	...
	/**
	 * The interface used to produce the visual representation of the
	 * {@link Location}.
	 * 
	 * <p>
	 * This interface is used to allow the {@link Visualisation} to create
	 * visual representations of a {@link Location}, by using the interface as a
	 * {@link FunctionalInterface} to use in the {@link Visualisation#visual};
	 * which allows the procedure of drawing a {@link Location} to be used in
	 * the {@link Visualisation}.
	 * </p>
	 * 
	 * @author John Berg
	 * @version 08/04/2017
	...
	public interface Visual {
		 ...
		 * 
		 * <p>
		 * Provide a {@link Graphics} and {@link Location} object which are the
		 * graphics to draw to and the {@link Location} to be drawn, along with
		 * an <code>int</code> x and y coordinate which specify the target
		 * location of where to draw.
		 * </p>
		 * 
		 * @param g
		 *            The {@link Graphics} add the visual representation to.
		 * @param loc
		 *            The {@link Location} to be visually represented.
		 * @param x
		 *            The x position of the {@link Visual}.
		 * @param y
		 *            The y position of the {@link Visual}.
		 * @see Graphics
		 * @see Location
		 */
		public void visiulise(final Graphics g, final Location loc, final int x, final int y);
	}
}